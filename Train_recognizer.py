...
batch_size_val= 6  # how many to process together
steps_per_epoch_val=2000
epochs_val=10
imageDimesions = (32,32,3)
testRatio = 0.2    # 20 % of positive images for testing
validationRatio = 0.2 #20% of the remaining 800 positives for validation (160)
#<=========================Import Images=========================>
count = 0
images = []
classNo = []
...
dataGen= ImageDataGenerator(width_shift_range=0.1,   # 0.1 = 10%     IF MORE THAN 1 E.G 10
                            height_shift_range=0.1, #THEN IT REFFERS TO NO. OF  PIXELS EG 10 PIXELS
                            zoom_range=0.2,  # 0.2 MEANS CAN GO FROM 0.8 TO 1.2
                            shear_range=0.1,  # MAGNITUDE OF SHEAR ANGLE
                            rotation_range=10)  # DEGREES
dataGen.fit(X_train)
batches= dataGen.flow(X_train,y_train,batch_size=20)  # REQUESTING DATA GENRATOR TO GENERATE IMAGES
X_batch,y_batch = next(batches)                       #BATCH SIZE = NO. OF IMAGES CREAED EACH TIME ITS CALLED
...
def myModel():
    ...
    model.add((Conv2D(no_Of_Filters,size_of_Filter,input_shape=(imageDimesions[0],imageDimesions[1],1),
                      activation='relu')))  # ADDING MORE CONVOLUTION LAYERS = LESS FEATURES BUT INCREASE ACCURACY
